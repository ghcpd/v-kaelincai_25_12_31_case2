...
class TestEventServiceSafariBrowserIssues:
    ...
    def test_registration_fails_with_us_date_format(self):
        ...
        TRIGGER: User submits date in US format with AM/PM
        EXPECTED: Successful registration
        ACTUAL: RegistrationError raised
        """
        service = EventService()
        
        # US format with AM/PM (common Safari manual entry) - 30 days in future
        future_date = datetime.now() + timedelta(days=30)
        us_date = future_date.strftime("%m/%d/%Y %I:%M %p")
...
class TestEventServiceFirefoxBrowserIssues:
    """
    Tests demonstrating Firefox browser compatibility issues - FIXED
    """
    
    def test_registration_fails_with_firefox_text_month(self):
        """
        FIXED: Firefox user with text month format now succeeds
        
        BUG LOCATION: src/date_parser.py (previously line 47-54)
        TRIGGER: User submits date with text month format
        EXPECTED: Successful registration
        ACTUAL: Registration succeeds after fix
        """
        service = EventService()
        
        # Text month format (common Firefox manual entry) - 30 days in future
        future_date = datetime.now() + timedelta(days=30)
        firefox_date = future_date.strftime("%b %d, %Y %H:%M")
        
        # This now works - registration should succeed
        result = service.register_participant(
            name="Firefox User",
    ...
    def test_registration_fails_with_past_date(self):
