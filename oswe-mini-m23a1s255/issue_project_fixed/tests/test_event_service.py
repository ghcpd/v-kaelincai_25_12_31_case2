...
All browser-format registration flows are expected to succeed with the
fixed date parser.
"""
...
class TestEventServiceSafariBrowserIssues:
    ...
    def test_registration_fails_with_safari_slash_format(self):
        """Safari user with slash-formatted date should be accepted"""
        service = EventService()
    ...
    def test_registration_fails_with_us_date_format(self):
        """US-locale Safari user with MM/DD/YYYY format should be accepted"""
        service = EventService()

        # US format with AM/PM (common Safari manual entry) — use a future date
        future_date = datetime.now() + timedelta(days=30)
...
class TestEventServiceFirefoxBrowserIssues:
    ...
    def test_registration_fails_with_firefox_text_month(self):
        """Firefox user with text month format should be accepted"""
        service = EventService()

        # Text month format (common Firefox manual entry) — use a future date
        future_date = datetime.now() + timedelta(days=30)
